...
const userManagement = async (req, res, db, actor) => {
  if (req.method === "GET") {
    ...
    return json(res, 200, { users: snap.docs.map((doc) => ({ id: doc.id, ...doc.data() })) });
  }
  ...
  const update = { updatedAt: now(), founderActionBy: actor.uid };
...
const coupons = async (req, res, db, actor) => {
  ...
  const type = body.type === "credits" ? "credits" : "percent";
  const value = Number(body.value || 0);
  const maxUses = Number(body.maxUses || 0);
  if (!code) return json(res, 400, { message: "Coupon code required." });
  if (!Number.isInteger(value) || value <= 0 || (type === "percent" && value > 100) || (type === "credits" && value > 100000)) return json(res, 400, { message: "Invalid coupon value." });
  ...
  if ((await ref.get()).exists) return json(res, 409, { message: "Coupon code already exists." });
  const data = { code, type, value, maxUses, usedCount: 0, expiresAt: body.expiresAt || null, active: body.active !== false, createdAt: now(), createdBy: actor.uid };
  await ref.set(data);
  await db.collection("adminAuditLogs").add({ actorUid: actor.uid, action: "createCoupon", targetUid: null, details: data, createdAt: now() });
...
const support = async (req, res, db, actor) => {
  ...
  if (!(await ref.get()).exists) return json(res, 404, { message: "Support message not found." });
  await ref.set({ adminReply: reply, status: String(body.status || "replied"), updatedAt: now(), repliedBy: actor.uid }, { merge: true });
  await db.collection("adminAuditLogs").add({ actorUid: actor.uid, action: "replySupport", targetUid: null, details: { supportId: id }, createdAt: now() });
  return json(res, 200, { ok: true, message: "Reply saved." });
};

export default async function handler(req, res) {
  const action = String(req.query?.action || req.body?.action || "health");
  try {
    const writeAction = ["config", "users", "support", "coupons"].includes(action) && req.method !== "GET";
    const actor = await requireFounderAdmin(req, { write: writeAction && action === "config" });
    const db = adminDb();
    ...
    if (req.method === "PUT" && action === "config") {
      const body = typeof req.body === "string" ? JSON.parse(req.body || "{}") : (req.body || {});
      if (!body.config || typeof body.config !== "object") return json(res, 400, { message: "A product configuration object is required." });
      const before = await getRuntimeProductConfig(db);
      const after = await saveRuntimeProductConfig({ db, config: body.config, actor: actor.uid });
      await writeConfigAudit({ db, actor, before, after, requestId: body.requestId });
