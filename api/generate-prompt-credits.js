...
class ApiError extends Error {
  constructor(status, code, message, quota) { super(message); this.status = status; this.code = code; this.quota = quota; }
}

const parseBody = (body) => {
  if (!body) return {};
  if (typeof body === "string") { try { return JSON.parse(body); } catch { throw new ApiError(400, "invalid_request", "The request body must be valid JSON."); } }
  return body;
};
...
const quotaFields = (quota) => ({
  ...
  imageAnalysesToday: quota.imageAnalysesToday,
  lastImageAnalysisDate: quota.lastImageAnalysisDate,
  imageAnalysesThisMonth: quota.imageAnalysesThisMonth,
  lastImageAnalysisMonth: quota.lastImageAnalysisMonth,
  quotaVersion: quota.quotaVersion,
  updatedAt: FieldValue.serverTimestamp(),
});

const reserve = async ({ db, uid, requestId, now, hasImage }) => {
  ...
  return db.runTransaction(async (tx) => {
    ...
    if (requestSnap.exists) {
      ...
      if (request.status === "succeeded" && request.prompt && request.quota) return { status: "succeeded", ...request };
      throw new ApiError(409, "request_in_progress", "This generation request is still being processed. Please wait a moment.");
    }
    const user = userSnap.exists ? userSnap.data() : {};
    const quota = createQuotaState(user, now);
    ...
    const creditCost = hasImage ? PRODUCT_CONFIG.creditCosts.referenceImageAnalysis : PRODUCT_CONFIG.creditCosts.standardGeneration;
    ...
    const nextState = {
      plan: quota.plan,
      promptsToday: usesFreePrompt ? quota.promptsToday + 1 : quota.promptsToday,
      lastPromptDate: dateKey,
      imageAnalysesToday: quota.imageAnalysesToday,
      lastImageAnalysisDate: quota.lastImageAnalysisDate,
      imageAnalysesThisMonth: quota.imageAnalysesThisMonth,
      lastImageAnalysisMonth: quota.lastImageAnalysisMonth,
      quotaVersion: quota.quotaVersion + 1,
    };
    if (hasImage) {
      if (quota.dailyImageLimit !== null) { nextState.imageAnalysesToday = quota.imageUsedToday + 1; nextState.lastImageAnalysisDate = dateKey; }
      else { nextState.imageAnalysesThisMonth = quota.imageUsedThisMonth + 1; nextState.lastImageAnalysisMonth = monthKey; }
    }
    const reservedQuota = createQuotaState(nextState, now);
...
const rollback = async ({ db, uid, requestId, now, failureCode }) => {
  const userRef = db.collection("users").doc(uid);
  const requestRef = userRef.collection("generationRequests").doc(requestId);
  const dateKey = getUtcDateKey(now);
  const monthKey = getUtcMonthKey(now);
  return db.runTransaction(async (tx) => {
    ...
    if (!requestSnap.exists || requestSnap.data().status !== "reserved") return createQuotaState(userSnap.exists ? userSnap.data() : {}, now);
    const reservation = requestSnap.data();
    const user = userSnap.exists ? userSnap.data() : {};
    const quota = createQuotaState(user, now);
    const restored = {
      plan: quota.plan,
      promptsToday: reservation.usesFreePrompt && reservation.dateKey === dateKey && quota.lastPromptDate === dateKey ? Math.max(quota.promptsToday - 1, 0) : quota.promptsToday,
      lastPromptDate: quota.lastPromptDate,
      imageAnalysesToday: quota.imageAnalysesToday,
      lastImageAnalysisDate: quota.lastImageAnalysisDate,
      imageAnalysesThisMonth: quota.imageAnalysesThisMonth,
    ...
    if (reservation.hasImage) {
      if (quota.dailyImageLimit !== null && reservation.dateKey === dateKey) restored.imageAnalysesToday = Math.max(quota.imageAnalysesToday - 1, 0);
      if (quota.monthlyImageLimit !== null && reservation.monthKey === monthKey) restored.imageAnalysesThisMonth = Math.max(quota.imageAnalysesThisMonth - 1, 0);
    }
    const restoredQuota = createQuotaState(restored, now);
    const restoredCredits = Math.max(Number(user.credits || 0), 0) + Math.max(Number(reservation.creditCost || 0), 0);
    tx.set(userRef, { ...quotaFields(restoredQuota), credits: restoredCredits }, { merge: true });
    tx.update(requestRef, { status: "rolled_back", failureCode, quota: restoredQuota, creditsRemaining: restoredCredits, completedAt: FieldValue.serverTimestamp() });
...
const generate = async ({ client, idea, aiModel, category, image }) => {
  ...
  let response;
  try { response = await call(PRIMARY_MODEL, 8000); } catch { response = await call(FALLBACK_MODEL, 7000); }
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const db = adminDb();
    const reservation = await reserve({ db, uid: decoded.uid, requestId: body.requestId, now: new Date(), hasImage: Boolean(input.image) });
    ...
    catch { const quota = await rollback({ db, uid: decoded.uid, requestId: body.requestId, now: new Date(), failureCode: "generation_failed" }); return json(res, 502, { code: "generation_failed", message: "The AI service could not generate a prompt. Your quota and credits were restored.", quota }); }
    ...
    const saved = await db.runTransaction(async (tx) => {
      ...
      if (!requestSnap.exists || requestSnap.data().status !== "reserved") throw new ApiError(409, "request_not_active", "The generation request is no longer active.");
      const request = requestSnap.data();
      tx.set(historyRef, { prompt: result.prompt, perspectives: result.perspectives, intelligence: result.intelligence, aiModel: body.aiModel, category: body.category, hasReferenceImage: Boolean(input.image), referenceImageName: input.image?.name || null, createdAt: FieldValue.serverTimestamp() });
      tx.update(requestRef, { status: "succeeded", prompt: result.prompt, perspectives: result.perspectives, intelligence: result.intelligence, historyId: historyRef.id, completedAt: FieldValue.serverTimestamp() });
      return { prompt: result.prompt, perspectives: result.perspectives, intelligence: result.intelligence, historyId: historyRef.id, quota: request.quota, creditsRemaining: reservation.creditsRemaining, creditCost: reservation.creditCost || 0 };
    });
    return json(res, 200, saved);
  } catch (error) {
    console.error("Creator credit generation failed", { code: error?.code || "unknown" });
    return json(res, error.status || 500, { code: error.code || "generation_unavailable", message: error.message || "Unable to generate a prompt right now.", ...(error.quota ? { quota: error.quota } : {}) });
  }
}
