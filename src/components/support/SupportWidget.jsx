      ...
      {open && (
        <div className="fixed bottom-24 right-5 z-[60] w-[min(92vw,380px)] overflow-hidden rounded-3xl border border-slate-700 bg-slate-950 text-white shadow-2xl shadow-slate-950/40">
          ...
          <div className="max-h-80 space-y-3 overflow-y-auto px-5 py-4">
            ...
            {user && messages.length === 0 && <div className="space-y-2"><p className="text-[11px] font-bold uppercase tracking-[0.14em] text-slate-600">Quick questions</p>{quickQuestions.map((item) => <button key={item.label} type="button" onClick={() => setMessage(item.message)} className="block w-full rounded-xl border border-slate-800 bg-slate-900 px-3 py-2.5 text-left text-xs font-semibold text-slate-300 hover:border-blue-500/40 hover:text-white">{item.label}</button>)}</div>}
