...
import ModeSelection from '../../components/builder/ModeSelection';
import StandardBuilder from '../../components/builder/StandardBuilder';
import ImageBuilder from '../../components/builder/ImageBuilder';
import CodingBuilder from '../../components/builder/CodingBuilder';
import { usePromptBuilder } from '../../hooks/usePromptBuilder';
import { useCodingBuilder } from '../../hooks/useCodingBuilder';
...
function PromptBuilder() {
  ...
  const codingBuilder = useCodingBuilder();
  ...
  const generationBlocked = mode === 'coding' ? false : creditBlocked;
  const needsPaidGeneration = mode === 'coding' ? false : (quotaExhausted || imageQuotaExhausted);
  ...
  useEffect(() => {
    if ((!generatedPrompt && !codingBuilder.generatedPrompt) || !resultRef.current) return undefined;
    const timer = window.setTimeout(() => {
      resultRef.current?.scrollIntoView({ behavior: 'smooth', block: 'start' });
    }, 80);
    return () => window.clearTimeout(timer);
  }, [generatedPrompt, codingBuilder.generatedPrompt]);

  const handleModeChange = (newMode) => {
    setSearchParams({ mode: newMode }, { replace: true });
    if (newMode === 'image') setCategory('image');
    else if (newMode === 'coding') setCategory('coding');
    else if (category === 'image' || category === 'coding') setCategory('writing');
  };
  ...
  return (
    <section id="prompt-builder" className="relative overflow-hidden bg-gradient-to-b from-slate-50 via-white to-blue-50/50 py-12 lg:py-20">
      ...
      <div className="relative mx-auto max-w-5xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="mt-8">
          <ModeSelection activeMode={mode} onModeChange={handleModeChange} disabled={isGenerating || isPreparingImage || codingBuilder.isGenerating || codingBuilder.isPreparing} />
        </div>
        ...
        <form
          className="mt-6 rounded-[1.5rem] border border-slate-200/80 bg-white/95 p-5 shadow-[0_24px_70px_-35px_rgba(37,99,235,0.35)] backdrop-blur sm:rounded-[2rem] sm:p-8"
          onSubmit={(e) => { e.preventDefault(); if (mode === 'coding') codingBuilder.generate(); else generate(); }}
          aria-busy={isGenerating || isPreparingImage || codingBuilder.isGenerating || codingBuilder.isPreparing}
        >
          {mode === 'standard' && (
            <StandardBuilder
              category={category}
          ...
          {mode === 'coding' && (
            <CodingBuilder
              idea={codingBuilder.idea}
              setIdea={codingBuilder.setIdea}
              images={codingBuilder.images}
              referenceFiles={codingBuilder.referenceFiles}
              handleFiles={codingBuilder.handleFiles}
              removeImage={codingBuilder.removeImage}
              removeFile={codingBuilder.removeFile}
              isPreparing={codingBuilder.isPreparing}
              isGenerating={codingBuilder.isGenerating}
              error={codingBuilder.error}
              message={codingBuilder.message}
            />
          )}
          ...
          <div className="mt-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <Button variant="primary" size="lg" type="submit" disabled={isGenerating || isPreparingImage || codingBuilder.isGenerating || codingBuilder.isPreparing || generationBlocked} className="w-full sm:w-auto">
              {isGenerating || codingBuilder.isGenerating ? (user ? 'Building…' : 'Signing in & building…') : workflow ? `Run ${workflow.name}` : mode === 'image' ? 'Create Image Prompt' : mode === 'coding' ? 'Generate Coding Prompt' : 'Generate Better Prompt'}
            </Button>
            <div className="text-right text-sm font-semibold" aria-live="polite">
              <p className={(needsPaidGeneration && credits >= generationCreditCost) ? 'text-emerald-700' : needsPaidGeneration ? 'text-red-600' : 'text-slate-600'}>
                {needsPaidGeneration ? (credits >= generationCreditCost ? `Using ${generationCreditCost} credits for this generation` : `Need ${generationCreditCost} credits to continue`) : `${quota.remaining} / ${quota.dailyLimit} prompts remaining today`}
              </p>
              {(image || mode === 'coding') && <p className="mt-1 text-xs text-slate-500">{mode === 'coding' ? 'Coding analysis: 5 credits per generation' : `Image analysis: ${generationCreditCost} credits after free usage`}</p>}
            </div>
          </div>
        </form>
        {quotaExhausted && plan === 'free' && mode !== 'coding' && <div className="mt-5 rounded-2xl border border-indigo-200 bg-gradient-to-r from-indigo-50 via-white to-violet-50 p-5 shadow-sm"><div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between"><div><p className="text-sm font-black text-slate-900">You've used today's {freeDailyLimit} free prompts.</p><p className="mt-1 text-xs text-slate-600">{credits > 0 ? `You have ${credits} credits available. Continue creating, or get more when you need them.` : 'Continue with Creator Credits or upgrade to Pro.'}</p></div><div className="flex flex-col gap-2 sm:flex-row"><Link to="/account#plans" className="rounded-xl bg-indigo-600 px-4 py-2.5 text-center text-xs font-bold text-white hover:bg-indigo-500">⭐ Upgrade · ₹79/month</Link><Link to="/account#plans" className="rounded-xl border border-indigo-200 bg-white px-4 py-2.5 text-center text-xs font-bold text-indigo-700 hover:bg-indigo-50">💎 Creator Credits</Link></div></div></div>}
        {imageQuotaExhausted && mode === 'standard' && <div className="mt-5 rounded-2xl border border-amber-200 bg-amber-50 p-5 text-sm text-amber-900"><strong>You've used your free reference-image allowance.</strong><p className="mt-1 text-xs">{credits >= generationCreditCost ? `You can continue now for ${generationCreditCost} credits. Paid image usage does not consume your free allowance.` : `You need ${generationCreditCost} credits to continue before the allowance resets.`}</p></div>}
        {(generatedPrompt || codingBuilder.generatedPrompt) && (
          <div ref={resultRef} className="mt-8 scroll-mt-24">
            <ResultCard
              prompt={mode === 'coding' ? codingBuilder.generatedPrompt : generatedPrompt}
              perspectives={mode === 'coding' ? codingBuilder.perspectives : perspectives}
              intelligence={mode === 'coding' ? codingBuilder.intelligence : intelligence}
              userPlan={plan}
              workflow={workflow}
              category={mode === 'coding' ? 'coding' : category}
            />
          </div>
        )}
      </div>
