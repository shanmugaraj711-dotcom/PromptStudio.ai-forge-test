...
function PromptBuilder() {
  ...
  const handleModeChange = (newMode) => {
    ...
    else if (category === 'image') setCategory('writing');
  };
  ...
  return (
    <section id="prompt-builder" className="relative overflow-hidden bg-gradient-to-b from-slate-50 via-white to-blue-50/50 py-12 lg:py-20">
      ...
      <div className="relative mx-auto max-w-5xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="mt-8">
          <ModeSelection activeMode={mode} onModeChange={handleModeChange} disabled={isGenerating || isPreparingImage} />
        </div>
        ...
        <form
          ...
          aria-busy={isGenerating || isPreparingImage}
        >
          {mode === 'standard' && (
            <StandardBuilder
              category={category}
          ...
          <div className="mt-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <Button variant="primary" size="lg" type="submit" disabled={isGenerating || isPreparingImage || generationBlocked} className="w-full sm:w-auto">
              {isGenerating ? (user ? 'Building…' : 'Signing in & building…') : workflow ? `Run ${workflow.name}` : mode === 'image' ? 'Create Image Prompt' : 'Generate Better Prompt'}
            </Button>
            <div className="text-right text-sm font-semibold" aria-live="polite">
              ...
              {image && <p className="mt-1 text-xs text-slate-500">Image analysis: {generationCreditCost} credits after free usage</p>}
            </div>
          </div>
        </form>
