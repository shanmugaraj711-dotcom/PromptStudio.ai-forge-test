...
const reserveOrderCreation = async ({ db, forgeRequestId, uid, amountInr, currency }) => {
  ...
  return db.runTransaction(async (tx) => {
    ...
    if (!["PAYMENT_REQUIRED", "ORDER_CREATING", "PAYMENT_PENDING"].includes(request.status)) {
      if (request.status === "PAID" || request.status === "PENDING_REVIEW" || request.status === "APPROVED" || request.status === "BUILDING" || request.status === "VERIFYING" || request.status === "READY") {
        return { action: "already_paid", request };
      }
      fail(409, "forge_request_not_payable", `Forge request is currently ${request.status}.`);
    }

    if (request.amountInr !== undefined && Number(request.amountInr) !== amountInr) {
      fail(409, "forge_price_changed", "The Forge price for this request no longer matches the current server configuration. Please start a new request.");
    }
    ...
    if (request.status === "ORDER_CREATING" && creatingAt && Date.now() - creatingAt < ORDER_TTL_MS) {
    ...
    tx.update(ref, {
      ...
      orderCreationAttempts: Number(request.orderCreationAttempts || 0) + 1,
      amountInr,
      currency,
      updatedAt: now(),
    });
...
const attachOrder = async ({ db, forgeRequestId, uid, order, amountInr, currency }) => {
  ...
  await db.runTransaction(async (tx) => {
    ...
    const request = snap.data() || {};
    if (request.razorpayOrderId && request.razorpayOrderId !== order.id) {
      fail(409, "forge_order_conflict", "A different Razorpay order is already linked to this Forge request.");
    }
    tx.update(ref, {
      status: "PAYMENT_PENDING",
      razorpayOrderId: order.id,
      razorpayReceipt: order.receipt || receiptFor(forgeRequestId),
      razorpayOrderCreatedAt: now(),
      orderCreationCompletedAt: now(),
      amountInr,
    ...
    const order = await razorpayRequest("/orders", {
      ...
      body: JSON.stringify({
        ...
        notes: {
          forgeRequestId,
          uid: user.uid,
          type: "apk_forge_build",
          priceSource: "PRODUCT_CONFIG.pricing.apkForge.buildPriceInr",
        },
