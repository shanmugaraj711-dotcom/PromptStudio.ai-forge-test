...
const fulfillForgeWebhook = async (orderId, paymentId) => {
  ...
  await db.runTransaction(async (tx) => {
    ...
    if (request.razorpayOrderId !== orderId) return;
    ...
    if (order.status !== "paid" || Number(order.amount) !== Number(request.amountPaise) || order.currency !== request.currency) return;
...
export default async function handler(req, res) {
  if (req.method !== "POST") return json(res, 405, { message: "Method not allowed." }); const rawBody = await readRawBody(req); const signature = req.headers["x-razorpay-signature"] || ""; if (!signature || !verifyWebhookSignature(rawBody, signature)) return json(res, 400, { message: "Invalid webhook signature." });
  const payload = JSON.parse(rawBody); const event = payload.event || ""; const eventId = req.headers["x-razorpay-event-id"] || `${event}:${payload?.payload?.payment?.entity?.id || payload?.payload?.subscription?.entity?.id || Date.now()}`; const db = adminDb(); const eventRef = db.collection("paymentWebhookEvents").doc(String(eventId).replace(/[^a-zA-Z0-9_-]/g, "_").slice(0, 150));
  try {
    ...
    if (event === "payment.captured") {
      ...
      if (payment?.order_id) {
        ...
        } catch (error) { console.error("Forge webhook order lookup failed", { code: error?.code || "unknown" }); }
    ...
    if (event === "payment.failed") { const payment = payload?.payload?.payment?.entity; await markCreditPaymentFailed(payment?.order_id, payment?.id); }
