...
const eventKey = (eventId) => String(eventId || "").replace(/[^a-zA-Z0-9_-]/g, "_").slice(0, 150);
...
const fulfillForgeWebhook = async (orderId, paymentId) => {
  ...
  if (order.status !== "paid" || Number(order.amount) !== Number(stored.amountPaise) || order.currency !== stored.currency) return;
  await db.runTransaction(async (tx) => {
    const snap = await tx.get(ref);
    if (!snap.exists) return;
    const request = snap.data() || {};
    if (request.razorpayOrderId !== orderId || Number(request.amountPaise) !== Number(order.amount)) return;
...
export default async function handler(req, res) {
  ...
  const eventId = suppliedEventId || `${event}:${payload?.payload?.payment?.entity?.id || payload?.payload?.subscription?.entity?.id || "unknown"}`;
  const db = adminDb();
  const eventRef = db.collection("paymentWebhookEvents").doc(eventKey(eventId));
  try {
    const existing = await eventRef.get();
    if (existing.exists && existing.data()?.status === "processed") return json(res, 200, { ok: true, duplicate: true });
    await eventRef.set({ event, eventId, receivedAt: existing.exists ? existing.data()?.receivedAt || new Date() : new Date(), status: "processing", updatedAt: new Date() }, { merge: true });
    if (event === "payment.captured") {
      const payment = payload?.payload?.payment?.entity;
      if (payment?.order_id) {
        const order = await razorpayRequest(`/orders/${encodeURIComponent(payment.order_id)}`);
        const isForge = order?.notes?.type === "apk_forge_build";
